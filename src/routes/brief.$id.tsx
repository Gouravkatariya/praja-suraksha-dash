...
import { RISK_DISCLAIMER, getPeers, getWork, tierColor, type RiskTier } from "@/lib/mplads/data";
import { useWorkDetailLive, useRiskFlagsForWorkLive } from "@/lib/mplads/live-data";

export const Route = createFileRoute("/brief/$id")({
  pendingMs: 0,
  pendingComponent: function BriefPending() {
    return (
      <AppShell title="Field Investigation Brief" subtitle="Preparing brief…">
...
function LiveBrief({ workId }: { workId: number }) {
  ...
  if (detailQ.isLoading) {
    return (
      <AppShell title="Field Investigation Brief" subtitle={`Work #${workId}`}>
        <div className="flex justify-center py-16">
          <Loader2 className="size-8 animate-spin text-muted-foreground" />
        </div>
      </AppShell>
  ...
  if (!detailQ.data) {
    return (
      <AppShell
        ...
        actions={
          <Link to="/projects/$id" params={{ id: String(workId) }} className={buttonVariants({ variant: "secondary" })}>
  ...
  const title = w.work_description ?? `Work #${w.work_id}`;

  return (
    <AppShell
      title="Field Investigation Brief"
      subtitle={`#${w.work_id} · prepared for field verification`}
      actions={
        <>
          <Link to="/projects/$id" params={{ id: String(w.work_id) }} className={buttonVariants({ variant: "secondary" })}>
            Back to risk profile
          </Link>
          ...
          <header className="border-b pb-4">
            <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4">
              ...
              <div className="shrink-0 text-right text-xs">
                <p className="font-mono">Ref: MoSPI/MPLADS/RI/{w.work_id}</p>
          ...
          <section>
            ...
            <table className="w-full text-sm">
              <tbody>
                {(
                  [
                    ["Work ID", String(w.work_id)],
                    ["Work number", w.unique_work_number],
                    ["Work name", title],
                    ["Category", w.category],
                    ["Constituency", w.constituency],
                    ["State", w.state],
                    ["Implementing agency / IDA", w.ida],
                    [
                      "Allocation",
                      w.allocation_amount != null
                        ? `₹${(w.allocation_amount / 100000).toFixed(2)} lakh`
                        : null,
                    ],
                    [
                      "Actual expenditure",
                      w.actual_expenditure != null
                        ? `₹${(w.actual_expenditure / 100000).toFixed(2)} lakh`
                        : null,
                    ],
                    ["Sanction date / due date", [w.sanction_date, w.expected_completion_date].filter(Boolean).join(" → ") || null],
                    ["Status", w.status],
              ...
              <div className="text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-mono text-xs text-muted-foreground">
                    {w.priority.flag_count} flags · highest {w.priority.highest_risk_level ?? "None"}
                  </span>
                </div>
                <p className="mt-1 text-muted-foreground">
                  Data quality {w.data_quality.score ?? "—"} · {flags.length} stored risk flags.
                </p>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            {flags.length === 0 ? (
              <p className="rounded-md border bg-muted/40 p-3 text-sm text-muted-foreground">
                No anomaly signal is stored for this work. Routine verification only; no adverse
                inference is drawn.
              </p>
            ) : (
                ...
                {flags.map((f, i) => (
                  <li key={f.id} className="rounded-md border p-3">
                    <p className="text-sm font-semibold">
                      {i + 1}. {f.risk_type}{" "}
                      <span className="font-mono text-xs text-muted-foreground">
                        ({f.risk_level} · {f.signal_origin})
                      </span>
                    </p>
                    <ul className="mt-1.5 space-y-1 text-xs text-muted-foreground">
                      {f.evidence.map((e, ei) => (
                        <li key={ei}>• {e.explanation}</li>
                      ))}
                    </ul>
                  </li>
                ))}
...
function Brief() {
  ...
  const { mode, persona } = usePlatform();
  const numericId = Number(id);

  if (mode === "live" && !id.startsWith("MP-") && !isNaN(numericId)) {
    return <LiveBrief workId={numericId} />;
  }
  ...
  return (
    <AppShell
      ...
      actions={
        <>
          <Link to="/projects/$id" params={{ id: work.id }} className={buttonVariants({ variant: "secondary" })}>
            Back to risk profile
          </Link>
          <Button onClick={() => window.print()}>
            <Printer className="mr-1 size-4" /> Print / export PDF
